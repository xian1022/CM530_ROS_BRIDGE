...
byte MoveBoth(word pos);

/*******************************************************************************
* Function Name  : MoveBoth
* Description    : Move ID9 and ID18 to same position
*******************************************************************************/
byte MoveBoth(word pos)
{
    ClearBuffer256();
    dxl_write_word(id1, P_GOAL_POSITION_L, pos);
    ...
    ClearBuffer256();
    dxl_write_word(id2, P_GOAL_POSITION_L, pos);
    CommStatus = dxl_get_result();
...
int main(void)
{
    ...
    TxDString("Power on -> both center 512\r\n");
    TxDString("Send 'a' -> both 200\r\n");
    TxDString("Send 'b' -> both 800\r\n");
    TxDString("Send 'c' -> both center 512\r\n");

    /* Power-on: go to center position first */
    if (MoveBoth(CENTER_POS))
    {
        TxDString("INIT -> ID9,ID18 = ");
        TxDWord16(CENTER_POS);
        TxDString("\r\n");
    }
    ...
    while (1)
    {
        if (CheckRxD_PC())
        {
            ...
            else if (pcCmd == 'c' || pcCmd == 'C')
            {
                if (!MoveBoth(CENTER_POS))
                    continue;

                TxDString("CMD C -> ID9,ID18 = ");
                TxDWord16(CENTER_POS);
                TxDString("\r\n");
                PrintErrorCode();
            }
