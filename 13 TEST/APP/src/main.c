...
* File Name          : main.c
* Author             : modified for dual AX motor trajectory testing
* Version            : V2.0.0 (integer AX position version)
* Date               : 2026/04/05
* Description        : CM-530 trajectory player for 2x AX-12A (integer version)
*
* 中文說明：
...
*   PING
*   HELP
*   BEGIN,<traj_id>,2,<point_count>
*   PT,<seq>,<dt_ms>,<id9_pos>,<id18_pos>
*   END,<traj_id>
*   STOP
*   HOME
*
...
/* Fixed joint count for this test project
 * Two AX motors: ID9 and ID18.
 */
#define JOINT_COUNT_FIXED       2
...
*
* Enable torque on the 2 test motors.
*******************************************************************************/
void InitApplication(void)
{
    int i;
...
/*******************************************************************************
* InitJointTable
* Description : Initialize 2 joints for dual motor test.
*
* 中文：
* Initialize ID and safety range for the 2 AX-12A motors.
...
{
    TxDString("\r\n========================================\r\n");
    TxDString("CM-530 Dual Motor Trajectory Test\r\n");
    TxDString("Integer AX Position Version\r\n");
    TxDString("Joint Order: j1->ID9, j2->ID18\r\n");
...
void PrintHelp(void)
{
    ...
    TxDString("PING\r\n");
    TxDString("HELP\r\n");
    TxDString("BEGIN,<traj_id>,2,<point_count>\r\n");
    TxDString("PT,<seq>,<dt_ms>,<id9_pos>,<id18_pos>\r\n");
...
/*******************************************************************************
* HandlePoint
* Format      : PT,<seq>,<dt_ms>,<id9_pos>,<id18_pos>
*
* 中文：
* 這是整數版軌跡播放的核心函式
* 1. 檢查 BEGIN 是否已收到
* 2. 檢查欄位數與 seq
* 3. Parse 2 AX position values
* 4. clamp 到安全範圍
* 5. 印出 target
* 6. Write targets to 2 AX-12A motors
* 7. 等待 dt_ms
...
*
* 中文：
* Move the 2 motors back to home_pos.
...
/*******************************************************************************
* MoveAllJoints
* Description : Write target position to all 2 joints in sequence.
*
* 中文：
* Write target positions to ID9 and ID18.
...
/*******************************************************************************
* ReadAllJoints
* Description : Read actual position from all 2 joints.
*
* Read actual positions from ID9 and ID18.
*******************************************************************************/
byte ReadAllJoints(JointState joints[], int count)
{
    int i;
